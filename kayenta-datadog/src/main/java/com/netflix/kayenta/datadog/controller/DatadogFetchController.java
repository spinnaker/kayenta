...
public class DatadogFetchController {
  ...
  public Map queryMetrics(
      ...
      @Parameter(schema = @Schema(defaultValue = "avg:system.cpu.user")) @RequestParam String metricName,
      @Parameter(description = "The scope of the Datadog query. e.g. autoscaling_group:myapp-prod-v002")
      ...
      @Parameter(description = "An ISO format timestamp, e.g.: 2018-03-15T01:23:45Z")
          @RequestParam(required = false)
          String end,
      @Parameter(schema = @Schema(defaultValue = "false")) @RequestParam(required = false) final boolean dryRun)
      throws IOException {
    // Apply defaults.
    scope =
