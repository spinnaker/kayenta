...
public class GraphiteConfiguration {
  ...
  MetricsService graphiteMetricsService(
    ...
    graphiteConfigurationProperties.getAccounts().stream()
        .filter(each -> each.getSupportedTypes().contains(AccountCredentials.Type.METRICS_STORE))
        .forEach(
            account -> {
              ...
              graphiteMetricsServiceBuilder.accountName(account.getName());
            });
