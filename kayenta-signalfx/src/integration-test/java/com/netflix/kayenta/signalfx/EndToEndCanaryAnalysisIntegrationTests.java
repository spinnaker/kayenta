...
@Slf4j
public class EndToEndCanaryAnalysisIntegrationTests extends BaseSignalFxIntegrationTest {
  ...
  public void test_that_a_canary_analysis_execution_can_be_created_healthy() throws InterruptedException {
    ...
    ValidatableResponse canaryAnalysisExRes =
        ...
        .when()
            .log().all(true)
            .post(String.format(getUriTemplate(), "/canary_analysis"))
        .then()
            .log().all()
            .statusCode(200);
    ...
    do {
      response = when().get(String.format(getUriTemplate(), "/canary_analysis/" + canaryAnalysisExecutionId))
          .then().statusCode(200);
      Thread.sleep(5000);
